...
export async function scrape(): Promise<void> {
  ...
  try {
    ...
    if (!mainResponse.ok) {
      throw new Error(
        `Failed to fetch main page: ${mainResponse.status} ${mainResponse.statusText}`,
      );
    }
    ...
    const urlMatch = mainHtml.match(
      /url:"(\\x2fc\\x2fportal\\x2frender_portlet[^"]*cotizacionfull_WAR_broutmfportlet[^"]*)"/,
    );
    if (!urlMatch) {
      throw new Error(
        'No portlet URL found in main page HTML. The page structure might have changed.',
      );
    }
    ...
    const portletResponse = await fetch(
      `https://www.brou.com.uy${portletUrl}`,
      {
        headers: {
          'User-Agent':
            'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
    ...
    if (!portletResponse.ok) {
      throw new Error(
        `Failed to fetch portlet: ${portletResponse.status} ${portletResponse.statusText}`,
      );
    }
    ...
    $('.cotizacion-portlet table tbody tr').each((i, el) => {
      ...
      if (moneda === 'Dólar' && compra && venta) {
        ...
        if (parsed.success) {
          ...
          log(
            'WARN [Dólar]',
            `Zod validation failed for ${moneda}: ${parsed.error.message}`,
          );
        }
      }
    });
    ...
    if (rows.length === 0) {
      throw new Error(
        'No "Dólar" data found in the table. Check selectors or currency name.',
      );
    }
    ...
    const newRecords = rows
      .map((row: ScrapedRow) => {
        const todayRecords = existingData.filter(
          (d) => d.fecha === fecha && d.moneda === row.moneda,
        );
        const lastRecord =
          existingData.length > 0
            ? existingData.filter((d) => d.moneda === row.moneda).pop()
            : null;
        ...
        if (
          isSameDayAsLast &&
          lastRecord?.compra === row.compra &&
          lastRecord?.venta === row.venta
        ) {
        ...
        const format = (val: number | null): string =>
          val !== null && !isNaN(val) ? val.toFixed(5) : '-';
    ...
    log(
      'SUCCESS [Dólar]',
      `Recorded new values for ${newRecords.length} currencies.`,
    );
  } catch (error: any) {
...
export function start(): void {
  log(
    'INFO [Dólar]',
    'Scheduling scraper to run every 15 minutes between 09:00 and 18:59 (Mon-Fri)...',
  );
  cron.schedule(
    '*/15 9-18 * * 1-5',
    () => {
