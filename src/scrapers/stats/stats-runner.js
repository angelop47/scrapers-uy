...
import { fetchStatsWithGemini } from './gemini-stats.js';

const TIMEZONE = 'America/Montevideo';
const STATS_DIR = path.join(process.cwd(), 'stats');

function writeStatsJson(newStats) {
...
export async function runStatsAutomation() {
            ...
            try {
                ...
                for (const file of files) {
                    ...
                    if (Array.isArray(data)) {
                        data.forEach(d => recentTitles.push(`${d.title} (${d.date})`));
                    }
                }
            } catch (e) {
